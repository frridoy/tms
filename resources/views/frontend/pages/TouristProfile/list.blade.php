        ...
        <div class="profile-container">
            <div class="left-column">
                ...
                <div class="user-info">
                    ...
                    <h6>Contact: {{ auth()->user()->contact }}</h6>
                    <hr>
                    <a class="btn btn-outline-warning" href="{{ route('touristprofile.edit', auth()->user()->id) }}"> Change
                        Your Info</a>
                </div>
            </div>
            <div class="right-column">
                @foreach ($userdata as $item)
                    <h6>Amount: {{ $item['amount'] }} BDT</h6>
                    <h6>Payment Status: {{ $item['payment_status'] }}</h6>
                    <h6>Transaction Id: {{ $item['transaction_id'] }}</h6>
                    <hr>
                @endforeach


                <a class="btn btn-outline-warning" href="{{ Route('my.booking', auth()->user()->id) }}"> My Booking
                    Info</a>
            </div>
        </div>
